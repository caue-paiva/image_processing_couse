...
\title{Relatório --- Filtros Convolucionais e Transformada de Fourier}
...
O código está contido em um único arquivo \texttt{trabalho3.py} e utiliza apenas \texttt{numpy}, \texttt{imageio} e \texttt{matplotlib}. Para executar:
...
\begin{lstlisting}
trabalho3/
  trabalho3.py              # Codigo principal (filtros + DFT manual)
...
O arquivo \texttt{trabalho3.py} está organizado em seções:
...
\caption{Imagem original --- escala de cinza}
\end{figure}
...
No domínio da frequência, o deslocamento espacial corresponde a multiplicar o espectro por uma exponencial complexa $e^{-j2\pi(u \cdot dx + v \cdot dy)/N}$. Isso altera apenas a fase dos coeficientes, mantendo a magnitude inalterada. Nenhum coeficiente é suprimido ou ressaltado --- o espectro de magnitude permanece idêntico ao da imagem original.
...
\textbf{Escolha de padding:} O reflect produziu um efeito similar ao do Box --- uma luz coerente nas bordas que se integra naturalmente com a iluminação da cena. Interessante como ambos os filtros de suavização respondem de forma similar ao reflect.
...
No domínio da frequência, o filtro Gaussiano é também uma Gaussiana --- um passa-baixa com transição suave e monotônica. Não há lobos laterais (sem ringing). As altas frequências são atenuadas exponencialmente conforme se afastam do centro do espectro. O parâmetro $\sigma$ controla a largura da banda passante: $\sigma$ maior no espaço $\rightarrow$ corte mais agressivo na frequência.
...
\textbf{Escolha de padding:} O wrap traz partes da outra borda da imagem (visível por exemplo no topo esquerdo). Mesmo sendo pequenas, essas regiões contrastam fortemente --- o branco delas contra o fundo preto cria um efeito visualmente legal e interessante.
...
\textbf{Escolha de padding:} O wrap produz efeito similar ao observado no Sobel --- artefatos de borda contrastantes que aparecem como linhas brancas vindas do lado oposto da imagem. Visualmente interessante pelo contraste que gera.
...
A resposta em frequência do Sharpen com Laplace é $H(u,v) = 1 + \alpha \cdot 4\pi^2(u^2 + v^2)$ --- mantém todas as frequências baixas (ganho unitário no centro) e amplifica progressivamente as altas frequências. O fator $\alpha$ controla a intensidade do realce. Diferente do Laplace puro que elimina as baixas frequências, o sharpen as preserva, apenas somando energia nas altas.
...
\textbf{Comparação:} Efeito similar ao Sharpen com Laplace, porém mais controlável --- permite ajuste independente do sigma (raio do blur) e do alpha (intensidade do realce). Produz resultado mais natural e suave, pois o Gaussiano usado internamente não gera artefatos de ringing.
...
\subsection{Emboss (Filtro Criativo --- Relevo)}
