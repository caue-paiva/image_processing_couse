...
\begin{figure}[H]
    ...
    \caption{Matriz de confusão para o melhor descritor individual.}
...
\begin{figure}[H]
    ...
    \caption{Matriz de confusão para a melhor combinação de descritores.}
...
O BoVW foi implementado sem usar OpenCV. A imagem foi dividida em patches densos de 16x16 pixels com passo 16. Para cada patch, foi extraído um histograma local de orientações de gradiente, complementado por média e desvio padrão de intensidade. Os descritores locais foram agrupados por K-Means com \(K=200\), e cada imagem foi representada por um histograma normalizado de visual words.
...
\begin{figure}[H]
    ...
    \caption{Exemplo de busca bem-sucedida: os primeiros retornos mantêm forte semelhança visual com a consulta.}
    \label{fig:success1}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Segundo exemplo de busca bem-sucedida, mostrando consistência visual entre consulta e retornos corretos.}
    \label{fig:success2}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Exemplo de falha de busca: imagens visualmente próximas em cor, textura ou fundo podem pertencer a classes diferentes.}
    \label{fig:failure1}
\end{figure}

Nos casos de sucesso, observa-se que a combinação de cor e textura consegue recuperar imagens em que o pet aparece com pelagem e contexto visual semelhantes. Nos casos de falha, os erros são explicáveis pela natureza dos descritores: eles não reconhecem semanticamente o animal, mas comparam padrões visuais. Assim, um pet diferente com cor, textura ou fundo parecido pode aparecer acima de imagens corretas mais diferentes em pose ou iluminação.
...
Na classificação, a melhor configuração foi GCH+LBP+GLCM+Gabor, com 38,30\% de acurácia no teste. Na busca, a melhor configuração foi GCH+LBP+GLCM, com mAP de 0.1346 e P@1 de 0.2970. Esses resultados indicam que cor e textura são as pistas mais importantes na base, enquanto forma por HOG foi menos confiável devido à variação de pose e ao alto número de dimensões.

O BoVW obteve mAP de 0.1073 e P@1 de 0.2262. Embora competitivo com alguns descritores isolados, ele ficou abaixo das melhores combinações globais. As visualizações confirmam uma distribuição com agrupamentos locais, mas forte sobreposição entre classes.
